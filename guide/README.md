...
We provide helper scripts to easily create the directory structure with the manifest.

- **On Windows**:
  ```powershell
  .\setup\windows\make_marker.ps1 -AppId 620 -Title "Portal 2" -OutDir .\disc_root
  ```
- **On Linux**:
  ```bash
  ./setup/linux/make_marker.sh 620 "Portal 2" ./disc_root
  ```

This creates a folder named `disc_root` containing `lastdisc.json` with the following content:
```json
{
  "app_id": "620",
  "title": "Portal 2",
